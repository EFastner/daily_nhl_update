...
ds.date_compile <- function(season_start, startdate = paste0(season_start,"-09-01"), enddate = paste0(season_start+1, "-07-31")) {
  ...
  df.game_numbers <- 
    df.game_list %>%
    filter(session == "R" | session == "P") %>% #Grab only regular and postseason
  ...
  return(list(df.pbp, df.roster))
}

fun.postgres_connect <- function(db.name, db.host, db.user, db.password){
